...
#if 1
	static bool _open( std::string name, Mode mode );
	static bool _read( void* buffer, int64_t size, int64_t& nin, int64_t maxChunkSize );
	static bool _write( const void* buffer, int64_t size, int64_t& nout, int64_t maxChunkSize );
	static bool _seek( int64_t pos );
	static bool _close();
	static int64_t _getSize();
#endif
