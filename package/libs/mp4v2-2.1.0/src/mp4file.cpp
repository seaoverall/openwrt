...
int MP4File::Get_m_file_status(void)
{
	if (m_file) {
		return 1;
	}
...
void MP4File::Open( const char* name, File::Mode mode, const MP4FileProvider* provider )
{
    //ASSERT( !m_file );
	if (!(!m_file)) {
		printf("already create file \r\n");
		return;
	}
	
	//printf("file name[%s]\r\n", name);
    m_file = new File( name, mode, NULL);
...
{
    if( !IsWriteMode() ) {
		printf("[%s:%d] operation not permitted in read mode \r\n", __FUNCTION__, __LINE__);
		return;
                                  ...
                                  MP4Property** ppProperty, uint32_t* pIndex)
{
    ...
    default:
        ...
        //throw new Exception(msg.str(), __FILE__, __LINE__, __FUNCTION__);
        printf("type mismatch - property %s type %d\r\n", name, (*ppProperty)->GetType());
    }
}
...
MP4TrackId MP4File::AddTrack(const char* type, uint32_t timeScale)
{
    ...
    //ASSERT(pTrakAtom);
	if (pTrakAtom == NULL) {
		printf("[%s:%d] AddChildAtom failed \r\n", __FUNCTION__, __LINE__);
    ...
    if (strlen(normType) > 4) {
		printf("[%s:%d] %s: type truncated to four characters \r\n", __FUNCTION__, __LINE__, GetFilename().c_str());
...
uint16_t MP4File::FindTrackIndex(MP4TrackId trackId)
{
    ...
    //throw new Exception(msg.str(),__FILE__, __LINE__, __FUNCTION__);
    //printf("[%s:%d] Track id %d doesn't exist \r\n", __FUNCTION__, __LINE__, trackId);
...
uint16_t MP4File::FindTrakAtomIndex(MP4TrackId trackId)
{
    ...
    //msg << "Track id " << trackId << " doesn't exist";
    //throw new Exception(msg.str(),__FILE__, __LINE__, __FUNCTION__);
    //printf("Track id %d doesn't exist \r\n", trackId);
    return (uint16_t)-1; // satisfy MS compiler
}
...
void MP4File::SetHintTrackSdp(MP4TrackId hintTrackId, const char* sdpString)
{
    ...
    if (strcmp(pTrack->GetType(), MP4_HINT_TRACK_TYPE)) {
		printf("track is not a hint track \r\n");
...
uint8_t MP4File::AllocRtpPayloadNumber()
{
    ...
    if (payload >= 128) {
		printf("no more available rtp payload numbers \r\n");
                         ...
                         bool isBframe, uint32_t timestampOffset)
{
    ...
    if (strcmp(pTrack->GetType(), MP4_HINT_TRACK_TYPE)) {
        //throw new Exception("track is not a hint track", __FILE__, __LINE__, __FUNCTION__);
		printf("[%s [%d:%s]]track is not a hint track", __FILE__, __LINE__, __FUNCTION__);
		return;
    }
