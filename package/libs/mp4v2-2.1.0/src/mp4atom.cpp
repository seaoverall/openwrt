...
MP4Atom* MP4Atom::FindAtom(const char* name)
{
	...
	//printf("[%s:%d] %s: FindAtom : matched %s \r\n",__FUNCTION__, __LINE__, GetFile().GetFilename().c_str(), name);
                           ...
                           MP4Property** ppProperty, uint32_t* pIndex)
{
    ...
    if (!IsRootAtom()) {
		...
		//printf("%s: FindProperty: matched %s \r\n", GetFile().GetFilename().c_str(), name);
                                    ...
                                    MP4Property** ppProperty, uint32_t* pIndex)
{
	...
	log.printf(MP4_LOG_INFO, "[%s:%d] FindProperty: no match for %s \r\n", __FUNCTION__, __LINE__, name);
    return false;
}
