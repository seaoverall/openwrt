...
{ int err = 1;
  ...
  { data->ctx = (void*)SOAP_MALLOC(soap, sizeof(char)*512);
    HMAC_CTX_reset((HMAC_CTX*)data->ctx);
  }
  else
  { data->ctx = (void*)SOAP_MALLOC(soap, sizeof(char)*128);
...
{ if (err <= 0)
  { unsigned long r;
    ...
    { if ((data->alg & SOAP_SMD_ALGO) == SOAP_SMD_HMAC)
        ...
        EVP_MD_CTX_reset((EVP_MD_CTX*)data->ctx);
      SOAP_FREE(soap, data->ctx);
      data->ctx = NULL;
    }
