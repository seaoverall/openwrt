	...
	IN PNDIS_PACKET pPacket)
{
	...
	)
	{
		   ...
		   )
		{
			...
			pAd->CommonCfg.MlmeTransmit.field.MODE = MODE_OFDM;
			pAd->CommonCfg.MlmeTransmit.field.MCS = MCS_RATE_6;
			//	Henry patch from Guilai for auth fail because driver always use bitrate-6 to send management-frame of sta
			//	but sometimes ap can not receive them because of low signal level such as 70~75dBm 
#ifdef P2P_SUPPORT
			...
			bss_idx = BssTableSearch(&pAd->ScanTab, pHeader_802_11->Addr1, pAd->LatchRfRegs.Channel);
			...
			if (bss_idx != BSS_NOT_FOUND)
			{
				if(bss_idx >= MAX_LEN_OF_BSS_TABLE)
				{
					...
					return NDIS_STATUS_FAILURE;
            			...
            			pBss = &pAd->ScanTab.BssEntry[bss_idx];
...
#ifdef RT_CFG80211_SUPPORT
if (pTxBlk->apidx >= MIN_NET_DEVICE_FOR_CFG80211_VIF_P2P_GO)
	pTxBlk->apidx = MAIN_MBSSID;
#endif /* RT_CFG80211_SUPPORT */
#ifdef P2P_SUPPORT
	...
	while (1)
	{
		...
		*/
		...
		if (!RTMP_CFG80211_VIF_P2P_GO_ON(pAd) && pAd->cfg80211_ctrl.isCfgInApMode == RT_CMD_80211_IFTYPE_AP)
			SET_PKT_OPMODE_AP(pAd);				
		else
			SET_PKT_OPMODE_STA(pAd);
#endif
		...
		{
			DBGPRINT(RT_DEBUG_INFO,("ACTION COME-IN !! TYPE = %d, IsReg = %d, %02x:%02x:%02x:%02x:%02x:%02x\n",
				  pHeader->FC.SubType, pAd->Cfg80211RegisterActionFrame, PRINT_MAC(pHeader->Addr2)));
		...
		if ( (pHeader->FC.Type == BTYPE_MGMT) && (pHeader->FC.SubType == SUBTYPE_DEAUTH) )
                {
                       DBGPRINT(RT_DEBUG_TRACE,("Deauth: %02x:%02x:%02x:%02x:%02x:%02x, %02x:%02x:%02x:%02x:%02x:%02x\n",
                                PRINT_MAC(pHeader->Addr1), PRINT_MAC(pHeader->Addr2)));
                }	
		...
		else
		{
			...
			else if(!INFRA_ON(pAd) && !RTMP_CFG80211_VIF_P2P_GO_ON(pAd)&& pAd->cfg80211_ctrl.isCfgInApMode == RT_CMD_80211_IFTYPE_AP)
			{
				bReschedule = APHandleRxDonePacket(pAd, pRxPacket, pRxBlk);			
			}
			else
...
BOOLEAN CmdRspEventCallbackHandle(PRTMP_ADAPTER pAd, PUCHAR pRspBuffer)
{
	...
	if (pFceInfo->self_gen)
	{
		...
		DBGPRINT(RT_DEBUG_INFO, ("Self-generated CMD Packet with event type=%d\n", pFceInfo->evt_type));
		(*CmdRspHandlerTable[pFceInfo->evt_type])(pAd, pRspBuffer + sizeof(*pFceInfo));
#else
