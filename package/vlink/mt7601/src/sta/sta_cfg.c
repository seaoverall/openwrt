...
static struct {
	...
	{"MonitorChBw",					Set_Monitor_Channel_And_Bw},
	...
	{"usbPowerSaving",                 Set_UsbPowerSaving},
	{"adv_debug",                      Set_AdvPowerSavingDebug},
#endif /* RTMP_MAC_USB */
	...
	/*AP & STA mode*/
	  {"Bw20RssiThr0",		  Set_BW20_RSSI_THR0_Proc},
	  {"Bw20RssiThr1",		  Set_BW20_RSSI_THR1_Proc},
	  {"Bw20RssiThr2",		  Set_BW20_RSSI_THR2_Proc},
	  {"Bw20RssiThr3",		  Set_BW20_RSSI_THR3_Proc},
	  {"Bw40RssiThr0",		  Set_BW40_RSSI_THR0_Proc},
	  {"Bw40RssiThr1",		  Set_BW40_RSSI_THR1_Proc},
	  {"Bw40RssiThr2",		  Set_BW40_RSSI_THR2_Proc},
	  {"Bw40RssiThr3",		  Set_BW40_RSSI_THR3_Proc},
	/*P2P mode*/
...
//yiwei cfg
			...
			DBGPRINT(RT_DEBUG_TRACE, ("CFG80211_MLME !!! MLME busy, reset MLME state machine !!! ORI ==> %lu\n", pAd->Mlme.CntlMachine.CurrState));
		...
		IN PSTRING				arg)
{
	...
	DBGPRINT(RT_DEBUG_ERROR, ("\x1b[31m  bInSuspendMode=  %d\x1b[m\n"
...
/* Sets Chip into Power-saving mode (low power-consumption) */
INT Set_UsbPowerSaving(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	PWR_SAVING_OP(pAd, RADIO_OFF, 5, 0, 0, 0, 0);
	return TRUE;
}

/* Bennett 20130329 debug adv power saving */
INT Set_AdvPowerSavingDebug(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	AndesPwrSavingOP(pAd, RADIO_OFF_ADVANCE, 0x1, 100, 1, 1, 0);
	return TRUE;
}
#endif /* RTMP_MAC_USB */
...
/*					kfree(pChListBuf); */
		...
		case OID_802_11_P2P_MODE:			
			...
			DBGPRINT(RT_DEBUG_TRACE, ("Query::OID_802_11_P2P_MODE (Len=%zu, Rule=%d)\n", sizeof(char),pAd->P2pCfg.Rule));
			break;
		...
		case OID_802_11_P2P_CTRL_STATUS:			
			...
			DBGPRINT(RT_DEBUG_TRACE, ("Query::OID_802_11_P2P_MODE (Len=%lu, DeviceName=%s)\n", pAd->P2pCfg.DeviceNameLen,pAd->P2pCfg.DeviceName));
			break;
		...
		case OID_802_11_P2P_SCAN_LIST:			
			...
			if (pUI_table)
			{
				...
				DBGPRINT(RT_DEBUG_TRACE, ("Query::OID_802_11_P2P_SCAN_LIST (Len=%zu, Rule=%d)\n", sizeof(char),pAd->P2pCfg.GroupOpChannel));
			...
			if ( sscanf(value, "%lx", &(rfValue)) == 1)
			{
				...
				if ( (sscanf((PSTRING)this_char, "%d", &(bank_Id)) == 1) && (sscanf((PSTRING)value, "%d", &(rfId)) == 1))
				{
					...
					if ( (rfId <= maxRFIdx) && (bank_Id <= MAC_RF_BANK) )
					{
						...
						sprintf(msg+strlen(msg), "BANK%d_R%02d:%02lX  ", bank_Id, rfId, rfValue);
					}
					else
					{
						bIsPrintAllRF = TRUE;
...
INT ed_status_read(RTMP_ADAPTER *pAd)
{
	...
	if ((pAd->ed_threshold > 0) && (period_us > 0) && (pAd->ed_block_tx_threshold > 0)) {
		...
		if (percent > 100)
			percent = 100;
		DBGPRINT(RT_DEBUG_TRACE, ("%s: ed_threshold %d, percent %d\n", __func__,
