...
void send_monitor_packets(IN PNET_DEV pNetDev,
	...
	*ph_11n33;		/* for new 11n sniffer format */
	...
	if (DataSize < pOSPkt->len) {
		...
		if ((DataSize - pOSPkt->len) < 
			(skb_end_pointer(pOSPkt) - skb_tail_pointer(pOSPkt))) {
	...
	IN VOID *pRatesOrg)
{
	...
	CFG80211DBG(RT_DEBUG_ERROR, ("80211> Number of channel = 0x%lu\n",
				(ULONG)CFG80211_NUM_OF_CHAN_5GHZ));
