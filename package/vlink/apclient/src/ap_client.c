...
static void wifi_site_survey(const char *ifname, int print)
{
	...
	struct iwreq wrq;
	char *line, *start;
	print_log("wifi_site_survey--------start\n");
	iwpriv(ifname, "SiteSurvey", "1");
	sleep(2);//hcw
	...
	close(socket_id);
	if (ret != 0) {
		print_log("wifi_site_survey--------out1\n");
		goto out;
	}

	if (wrq.u.data.length < 1) {
		print_log("wifi_site_survey--------out2\n");
		goto out;
	}
	...
	line = strtok(NULL, "\n");
	survey_count = 0;
	print_log("wifi_site_survey--------while start\n");
	while(line && (survey_count < 64)) {
		...
		if (st[survey_count].crypto) {
			*st[survey_count].crypto = '\0';
			...
			print_log("Found network - channel:%s,ssid:%s,bssid:%s, security:%s\n",
		...
		survey_count++;
		print_log("survey_count++:%d\n",survey_count);
	}
	print_log("wifi_site_survey--------while end\n");
...
//英文转ascii码，中文转url编码
//Ch  SSID i大家好                       BSSID               Security               Siganl(%)W-Mode  ExtCH  NT WPS DPID
//1   0x69E5A4A7E5AEB6E5A5BD           94:0e:6b:67:4e:0b   WPA2PSK/AES            100      11b/g/n NONE   In  NO  
...
//3: find hidden ssid
	...
	print_log("find total_hide_num=%d,sq_hide_num=%d, survey_count=%d\n",total_hide_num,sq_hide_num,survey_count);
	for (i = 0; i < survey_count; i++)
	{
			if (!strcmp(" ", (char*)st[i].ssid))
				{
					if(j++ == sq_hide_num)
					{
						print_log("find i=%d,total=%d,sq=%d\n",i,total_hide_num,sq_hide_num);
						sq_hide_num++;
						return &st[i];
					}
				}
	}
	print_log("find hidden ssid return 0\n");
	return 0;
}
				...
				const char *bssid, const char *key, const char *enc, const char *crypto)
{
	//char buf[100];
	int enctype = 0;
	print_log("wifi_repeater_start (essid: %s, key: %s, bssid: %s, channel: %s, enc: %s, crypto: %s)\n",
					ssid, key, bssid, channel, enc, crypto);
	...
	} else if (strstr(enc, "NONE") || !key || key[0] == '\0') {
		print_log("ApCliAuthMode------------ NONE\n");
		iwpriv(staname, "ApCliAuthMode", "NONE");
	} else {
		print_log("ApCliAuthMode------------ return\n");
		return;
	}
...
static int g_apclient_connect_flag = 0;
//                          ra0           apcli0        ssid          pass
static void assoc_loop(char *ifname, char *staname, char *essid, char *pass, char *bssid)
{
	...
	while (1) {
		print_log("check:");
		if ((is_connect == 0) && (check_assoc(staname) == 0)) {
			...
			if (c) {
				int i = 20;
				print_log("Found network, trying to associate (essid: %s, bssid: %s, channel: %s, enc: %s, crypto: %s)\n",
				...
				while(i--) {
					if (!check_assoc(staname)) {
						print_log("while---check-----not connect\n");
					} else {
						print_log("while---check-----connect\n");
						if (g_apclient_connect_flag == 0) {
							g_apclient_connect_flag = 1;
						}
						break;
					}
					sleep(1);
				...
				print_log("No signal found to connect to ssid:%s\n", essid);
				sleep(3);
			}
		} else {
			print_log("connect\n");
...
//	led_set_trigger(1);
	print_log("loop:%s,%s,%s,%s,%s,%s,%s,%s\n",argv[1], argv[2], argv[3], argv[4], argv[5]);
